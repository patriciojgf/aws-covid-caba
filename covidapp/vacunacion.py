...
df_s = []
txt = pd.read_csv('https://sisa.msal.gov.ar/datos/descargas/covid-19/files/datos_nomivac_covid19.zip',encoding = "utf-8",compression='zip',sep=',',chunksize=100000,usecols=['fecha_aplicacion','sexo','grupo_etario','orden_dosis','vacuna','jurisdiccion_residencia_id'],dtype=str)
for df in txt:
    df_s.append(df[df['jurisdiccion_residencia_id']=='02'])
df_f = pd.concat(df_s,ignore_index = True)
df_s = []
df_f['juridiccion_residencia_id'] = df_f['jurisdiccion_residencia_id'].astype(int)
df_f['orden_dosis'] = df_f['orden_dosis'].astype(int)
df_f['fecha_aplicacion'] = pd.to_datetime(df_f['fecha_aplicacion'])
df2=df_f
df2.rename(columns={'fecha_aplicacion':'FECHA','sexo':'GENERO_ID','grupo_etario':'GRUPO_ETARIO_ID','orden_dosis':'DOSIS','vacuna':'VACUNA_ID','juridiccion_residencia_id':'COMUNA_ID'}, inplace=True)
...
df2 = df2.groupby(['FECHA','GENERO_ID','GRUPO_ETARIO_ID','DOSIS','VACUNA_ID','jurisdiccion_residencia_id']).size().reset_index(name='CANTIDAD')
